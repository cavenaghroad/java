...
//				text:'Add New',
//				click:function(){
//					$.post('_xdbwork.php',{optype:'addnewparty',partyname:$('#partyname').val(),sector:$('#selSector').val()},function(json){
//						console.log(json);
//						if(json['result']!='0'){
...
.on('keydown','#userid,#passcode',function(e){
	switch( e.keyCode || e.which )	{
//	case KEY_TAB:
	case KEY_ENTER:
		if(this.id=='userid') $('#passcode').select();
		if(this.id=='passcode') $('#btnLogin').click();
//		return false;
	default:
...
.on('click','#btnLogin',function(e){
	...
	$.post('_svlogin.php',{
		...
		$.cookie('ezra_admin',$('#userid').val(),{expires:10000});
//		$.cookie('ezra_passcode',$('#passcode').val(),{expires:10000});
//		$('#selParty').empty();
//		$.each(json['party'],function(ndx,val){
		console.log('party ['+json['party']+']');
		var pstr=json['party'].split(',');
		$.post('_svwork.php',{optype:'svctl',_e:pstr[0]},function(json){
			if(json['result']!='0'){
				alert(json['msg']); return false;
			}
//			document.location='crmctl.php?_p='+json['firstpage'];
			document.location='svctl.php?_p='+json['firstpage'];
		},'json');
...
.on('change','#selParty',function(){
	...
	if($(this).val()!='new'){
		var me=$(this);
		alert('party ['+me.val()+']');
		$.post('_xdbwork.php',{optype:'crmctl',_e:me.val()},function(json){
			if(json['result']!='0'){
				alert(json['msg']); return false;
			}
			document.location='crmctl.php?_p='+json['firstpage'];
...
function setLang(pstr){
	...
	document.location=BaseFolder+'/sv.php?_l='+pstr;
}
*/
