...
try {    
    ...
    while($row=$rs->fetch_assoc()){        
        errorlog("{$n} {$row['tname']}.{$row['fname']} pixel [{$row['pixel']}] _join [{$row['_join']}] dtype [{$row['dtype']}]",__LINE__,__FUNCTION__,__FILE__);
    ...
    errorlog("orderby [".implode(",",$orderby)."]",__LINE__,__FUNCTION__,__FILE__);
    ...
    foreach($colinfo as $n=>$v){
        ...
        errorlog("query_key [{$n}] [{$query_key[count($query_key)-1]}]",__LINE__,__FUNCTION__,__FILE__);
    }
    // INNER,OUTER JOIN
    ...
    foreach($colinfo as $n=>$v){
        ...
        errorlog("_join [{$v['_join']}] [{$n}]",__LINE__,__FUNCTION__,__FILE__);
    }
    $from=$prev_col="";
    array_multisort($join_key,$join_ndx);
    foreach($join_key as $n=>$val){
        errorlog("join_key [{$val}] [".substr($val,-1)."]",__LINE__,__FUNCTION__,__FILE__);
        $x=$join_ndx[$n];
        if(substr($val,-1)=="+"){
        ...
        errorlog("from [{$from}]",__LINE__,__FUNCTION__,__FILE__);
    }
    if(count($join_key)<1){
...
//     throw new Exception("do here.");
    ...
    case "HRZN":
        ...
        errorlog("HRZN [{$sql}]",__LINE__,__FUNCTION__,__FILE__);
        ...
        while($row=$rs->fetch_assoc()){
            ...
            if($row['fname']=="inactivated" && $row['tname']=="i_col"){
                //errorlog("ROW [{$pstr}]",__LINE__,__FUNCTION__,__FILE__);
            }
            if($ndx==1){
        ...
        break;
    case "TREE":
        errorlog("TREE [{$sql}]",__LINE__,__FUNCTION__,__FILE__);
        
        //             $sql=str_replace("order by","where par_rowid=party order by",$sql);
        $skip_col=3;
        $answer['tree']=array();
        errorlog(">>>> buildTree start [{$row['rowid']}]",__LINE__,__FUNCTION__,__FILE__);
    ...
    case "VERT":
        if($pagesize!="0") $sql.=" limit {$start},{$pagesize}";
        errorlog("VERT [{$sql}]",__LINE__,__FUNCTION__,__FILE__);
...
//         errorlog("{$sql} [{$rs->num_rows}]",__LINE__,__FUNCTION__,__FILE__);
        ...
        while($row=$rs->fetch_assoc()){
            ...
            foreach($row as $k=>$val){
                errorlog("key [{$k}] val [{$val}] pixel [{$pixel[$k]}] align [{$align[$k]}]",__LINE__,__FUNCTION__,__FILE__);
        ...
        errorlog(implode("/",$vert),__LINE__,__FUNCTION__,__FILE__);
...
// //     errorlog("[TREE HTML] {$pstr}",__LINE__,__FUNCTION___,__FILE__);
...
//     errorlog("tname [{$tname}]/join [{$joinstr}]/orderby [".implode(",",$orderby)."]/par_rowid [{$par_rowid}]",__LINE__,__FUNCTION__,__FILE__);
    errorlog("column [{$column}] from [{$from}] query [".implode(" and ",$query_key)."] order by [".implode(",",$orderby)."] par_rowid [{$par_rowid}]",__LINE__,__FUNCTION__,__FILE__);
//     array_unshift($query_key,"par_rowid='{$par_rowid}'");
    ...
    while($row=$rs->fetch_assoc()){
        ...
        errorlog("line [".htmlentities($tag['line'])."]",__LINE__,__FUNCTION__,__FILE__);
