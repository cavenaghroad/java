...
.on('click','#selClass option',function(){
	...
	$.post('_ncs_student.php',oParam,function(result){
		...
		$('#bAlive').attr('checked',false);
		if(data['alive']=='1'){
			$('#bAlive').attr('checked',true);
		} 
	},'json');
	return false;
})
...
.on('click','#btnLink',function(){
	if($('#selClass option:selected').val()!='') {
		document.location="http://3.134.55.142/ncs/ncs_sv.php?name=a2z4sg&class="+$('#selClass option:selected').val();
	}
	return false;
})
...
//		}
		...
		$('#tvid').val(rec['tvid']);
		$('#seq').val(rec['seq']);
		if(rec['alive']=='1') $('#cAlive').attr('checked',true);
