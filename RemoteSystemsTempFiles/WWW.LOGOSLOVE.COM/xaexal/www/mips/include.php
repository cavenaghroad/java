...
function sqlrun($sql,$ln="",$fn="",$sfile=""){
    ...
    $wlog->write("[{$sql}] [{$cnt}] [{$msc}sec]",$ln,$fn,$sfile);
    return $rs;
}
...
function MainMenu($party){
    global $wlog;
    
    echo "<ul id=mainmenu style='cursor:pointer'>";
    $wlog->write(">>> Building Left menu.",__LINE__,__FUNCTION__,__FILE__);
    $rs=sqlrun("select menu_id from a_party where party='{$party}'",__LINE__,__FUNCTION__,__FILE__);
    $row=$rs->fetch_assoc();
    $pstr= showMenu($row['menu_id']/*$_SESSION['party']*/);
    echo $pstr;
...
function showMenu($rowid,$par_rowid=""){
    $rs=sqlrun("select * from i_navi where par_rowid='{$rowid}' order by rowid,par_rowid,seqno",__LINE__,__FUNCTION__,__FILE__);
    $pstr="";
...
function drawLayout($_p){
    ...
    $wlog->write(">>> Building Page.",__LINE__,__FUNCTION__,__FILE__);
    ...
    $wlog->write(">>> Building Grid.",__LINE__,__FUNCTION__,__FILE__);
    ...
    $n=1;
    while($row=$rs->fetch_assoc()){
        $wlog->write(">>> Building Column",__LINE__,__FUNCTION__,__FILE__);
                ...
                while($rowCol=$rsCol->fetch_assoc()){
                    switch($rowCol['fname']){
                        ...
                        default:
                            ...
                            //     $wlog->write("list [{$strList}]",__LINE__,__FUNCTION__,__FILE__);
...
//         if($btndelete!="") $btndelete.="&nbsp;<button class='ui-icon ui-icon-refresh' style='width:20px;height:20px' id=reload{$row['rowid']} />";  link breaking issue happens.
        ...
        if(strpos($pstr,"{{$n}}")===false) {
            $wlog->write("{{$n}} does not exist.",__LINE__,__FUNCTION__,__FILE__);
        } else {
