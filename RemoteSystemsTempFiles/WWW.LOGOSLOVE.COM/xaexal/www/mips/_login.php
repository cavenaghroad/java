...
try{
	...
	$wlog->write("member [{$_SESSION['member_id']}] member_name [{$_SESSION['member_name']}] startpage [{$startpage}]",__LINE__,__FUNCTION__,__FILE__);
...
// 		$sql = "update a_member set last_login='{$date->format('YmdHis')}',login_region='{$country}' where member_id='{$_SESSION['member_id']}'";
    $rs=sqlrun("update a_member set last_login=now() where member_id='{$_SESSION['member_id']}'",__LINE__,__FUNCTION__,__FILE__);
	...
	$wlog->write("{$answer['msg']} -> [{$sql}]",__LINE__,__FUNCTION__,__FILE__);
}
