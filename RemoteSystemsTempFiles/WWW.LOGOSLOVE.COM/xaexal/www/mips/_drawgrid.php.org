...
//                 $wlog->write("{$k} [{$dtype[$k]}]",__LINE__,__FUNCTION__,__FILE__);
                if($v!=strip_tags($v)) {    // this column has HTML tags
                    $pstr.="<td {$tdStyle}>".htmlentities($v)."</td>";
                } else if($dtype[$k]=="bool"){
//                     $wlog->write("{} bool v [{$v}]",__LINE__,__FUNCTION__,__FILE__);
                    $pstr.="<td {$tdStyle}><input type=checkbox ".($v=="1"?"checked":"");
//                     if($colInfo[$kndx-1]['editable']!="1") $pstr.=" disabled";
                    $pstr.="></td>";
//                     $wlog->write("{$k} [{$pstr}]",__LINE__,__FUNCTION__,__FILE__);
                } else if($v==""){
                    $pstr.="<td {$tdStyle}>&nbsp;</td>";
                } else {
