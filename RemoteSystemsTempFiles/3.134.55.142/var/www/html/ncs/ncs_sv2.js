...
$(document)
.ready(function(){
	$('td[id^=s]').each(function(){
		$(this).find('.lu').prepend(this.id.substr(1));
	});
	$.post('_ncs_sv.php',{optype:'init'},function(json){
		if(json['result']!='0'){
			alert(json['msg']);
			return false;
		}
...
.on('click','table[id^=s] td',function(){
	var col=$(this).index();
...
.on('click','#btnAdd',function(){
	$.ajax({
		...
		data:'optype=add&name='+$('#txtDrill').val(),
		beforeSend:function(){
...
.on('click','#btnDel',function(){
	...
	$.post('_ncs_sv.php',{optype:'delete',name:$('#txtDrill').val()},function(json){
		if(json['result']!='0'){
			alert(json['msg']); 
			$('#txtDrill').val('');
			return false;
...
.on('click','#btnDone',function(){
	...
	$.post('_ncs_sv.php',{optype:'done',student:student,drill:$('#selDrill').val()},function(json){
		if(json['result']!='0'){
			alert(json['msg']); 
			return false;
		}
...
.on('click','#btnReset',function(){
	var me=$(this);
	var student=me.closest('table').find('tr:eq(0) td:eq(0)').text();
	$.post('_ncs_sv.php',{optype:'setmission',status:'reset',student:student,drill:$('#selDrill').val()},function(json){
		if(json['result']!='0'){
			alert(json['msg']); 
			return false;
		}
...
.on('click','#lblSeat',function(){
	...
	$.post('_ncs_sv.php',{optype:'changeseat',name:name,oldseat:oldseat,newseat:str},function(json){
		if(json['result']!='0'){
...
.on('blur','input[name=ipaddr]',function(){
    ...
    $.ajax({
        url:'_ncs_sv.php',
        data:'optype=ipaddr&seq='+pstr+'&ipaddr='+$(this).val(),
        method:'post',
        dataType:'json',
        beforeSend:function(){
...
function getDrill(){
	...
	// });
	$.post('_ncs_sv.php',{optype:'getdrill',drill:$('#selDrill').val()},function(json){
		var done=[]; var submit=[];
...
function drillList(){
	$.post('_ncs_sv.php',{optype:'drill'},function(json){
