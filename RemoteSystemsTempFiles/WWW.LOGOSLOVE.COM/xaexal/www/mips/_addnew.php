...
try{
    ...
    */
    case "TREE":
        ...
        $wlog->write("ROWID [{$rowid}] PAR_ROWID [{$par_rowid}]",__LINE__,__FUNCTION__,__FILE__);
...
//         foreach($rs as $i=>$row){
        ...
        while($row=$rs->fetch_assoc()){
            $wlog->write("i [{$n}] fname [{$row['fname']}] dtype [{$row['dtype']}] unique [{$row['unique_key']}] default [{$row['defvalue']}] childCol [{$childCol}]",__LINE__,__FUNCTION__,__FILE__);
            ...
            $wlog->write("{$row['fname']} defvalue [{$row['defvalue']}]",__LINE__,__FUNCTION__,__FILE__);
            ...
            default:
                ...
                case "unique":  // unique value for rowid or other unique key.
                    $tagvalue=getROWID();
                    $sqlvalue="'{$tagvalue}'";
                    $wlog->write("_join [{$row['_join']}] defvalue [{$sqlvalue}]",__LINE__,__FUNCTION__,__FILE__);
                    break;
                default:
            ...
            $wlog->write("tagvalue [{$tagvalue}] sqlvalue [{$sqlvalue}]",__LINE__,__FUNCTION__,__FILE__);
            if($row['_join']=="0"){
                $arSQL[$row['tname']].="{row['fname']}={$sqlvalue}";
                $wlog->write("{$row['fname']}={$sqlvalue}",__LINE__,__FUNCTION__,__FILE__);
            } else {
    ...
    case "HRZN":
        ...
        while($row=$rs->fetch_assoc()){
            $i++;
            $wlog->write("fname [{$row['fname']}] dtype [{$row['dtype']}] unique [{$row['unique_key']}] defvalue [{$row['defvalue']}] ndx [{$ndx}] childCol [{$childCol}]",__LINE__,__FUNCTION__,__FILE__);
            // when new row has different table name, add new element to arSQL and set INSERT statement.
            $tname=$row['tname'];
            $fname=$row['fname'];
            ...
            $wlog->write("{$tname} [{$arSQL[$tname]}]",__LINE__,__FUNCTION__,__FILE__);
    ...
    $wlog->write("TAG [".htmlentities($answer['html'])."]",__LINE__,__FUNCTION__,__FILE__);
    $answer['result']=0;
    $mysqli->commit();
} catch(Exception $e){
    $mysqli->rollback();
    if($e->getMessage()=="root"){
        ...
        $wlog->write($pstr,__LINE__,__FUNCTION__,__FILE__);
