...
try {
    $sql="select viewtype from i_grid where rowid='{$rowid}'";
    $rs=sqlrun($sql,__LINE__,__FUNCTION__,__FILE__);
    if($rs->num_rows<1) throw new Exception("No Grid info");
    $row=$rs->fetch_assoc();
    $viewtype=$row['viewtype'];
    
    $sql="select * from i_col where par_rowid='{$rowid}' and inactivated='0'  order by seqno";
    $rs=sqlrun($sql,__LINE__,__FUNCTION__,__FILE__);
