...
if(!isset($_GET['class'])){
    $sql="select classcode from ncs_config where period2 =(select max(period2) from ncs_config)";
    $rs=$mysqli->query($sql);
    $row=$rs->fetch_assoc();
    $_GET['class']=$row['classcode'];
}
?>
...
<tr><td valign=top>
	<table width=100%>
	<tr>
		<td colspan=2><a href='ncs_student.php?name=a2z4sg'>학생명단관리</a></td>
	</tr>
	<tr><td colspan=2>
...
<?
	...
	if( $seat_cnt % $col_cnt==0){
		for($i=1;$i<=$seat_cnt;$i++){
			...
			$pstr="<td class=student valign=top><table width=100% height=100% id=tbls".$i."></table></td>".$pstr;
